alter table cali_workspace.profiles
  alter column signature_style set default 'executive';
...
alter table cali_workspace.profiles
  ...
  check (signature_style in (
    'executive','editorial','notarial','heritage','calligraphic','autograph','contemporary','italian','minimal','personal',
    'classic','fluid','delicate','formal'
  ));

update cali_workspace.profiles
set signature_style=case signature_style
  when 'classic' then 'heritage'
  when 'fluid' then 'calligraphic'
  when 'delicate' then 'contemporary'
  when 'formal' then 'executive'
  else signature_style
end
where signature_style in ('classic','fluid','delicate','formal');
...
create or replace function cali_workspace.update_my_profile_v56(
  ...
  p_signature_style text default 'executive'
  ...
  if new.status='approved' and old.status is distinct from 'approved' and new.approval_identity_snapshot is null then
    ...
    if p.id is not null then
      new.approval_identity_snapshot=jsonb_build_object(
        ...
        'signature_mode',coalesce(p.signature_mode,'generated'),'signature_url',p.signature_url,'signature_style',coalesce(p.signature_style,'executive'),
