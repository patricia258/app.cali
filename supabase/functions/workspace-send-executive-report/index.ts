...
function html(company:string,period:string,message:string,reportId:string){
  ...
  const personal=message?`<div style="margin:20px 0 24px;padding:15px 17px;border-left:3px solid #B58C52;background:#FBF7F3;border-radius:0 10px 10px 0"><div style="font-size:10px;letter-spacing:.1em;text-transform:uppercase;color:#B58C52;font-weight:700;margin-bottom:7px">Uma observação minha</div><div style="font-size:14px;line-height:1.65;color:#3C3531">${esc(message).replaceAll("\n","<br>")}</div></div>`:"";
  return `<!doctype html><html lang="pt-BR"><body style="margin:0;background:#F7F3EE;font-family:Arial,Helvetica,sans-serif;color:#2B2B2B"><table width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:28px 14px"><table width="100%" cellpadding="0" cellspacing="0" style="max-width:640px;background:#fff;border:1px solid #E8DDD4;border-radius:18px;overflow:hidden"><tr><td style="padding:20px 26px;border-bottom:1px solid #6A2838;background:#5A1E2D"><table width="100%" cellpadding="0" cellspacing="0"><tr><td align="left" valign="middle"><img src="${LOGO_URL}" alt="CALI Workspace" width="122" style="display:block;width:122px;max-width:122px;height:auto"></td><td align="right" valign="middle"><img src="${MARK_URL}" alt="" width="48" height="48" style="display:block;width:48px;height:48px;object-fit:contain"></td></tr></table></td></tr><tr><td style="padding:30px"><div style="font-size:10px;letter-spacing:.12em;color:#B58C52;font-weight:700;margin-bottom:8px">FECHAMENTO CALI · ${esc(period)}</div><h1 style="font-size:27px;line-height:1.22;font-weight:750;letter-spacing:-.02em;color:#5A1E2D;margin:0 0 18px">Seu relatório de ${esc(period)} está pronto.</h1><p style="font-size:15px;line-height:1.7;margin:0 0 15px">Olá,</p><p style="font-size:15px;line-height:1.7;margin:0 0 15px">Fechei a leitura de ${esc(period)} e deixei o relatório da ${esc(company)} disponível no Workspace.</p>${personal}<p style="font-size:15px;line-height:1.7;margin:0 0 15px">Você pode abrir o documento quando quiser e baixar o PDF para guardar. Se fizer sentido para você, também dá para registrar sua ciência de leitura — esse registro é opcional.</p><p style="font-size:15px;line-height:1.7;margin:0 0 15px">Se depois da leitura surgir alguma dúvida, você pode registrá-la no Workspace por até 3 dias úteis após a primeira abertura.</p><p style="font-size:15px;line-height:1.7;margin:0 0 25px">Se algum ponto merecer conversa, me chama.</p><div style="text-align:center;margin:0 0 27px"><a href="${url}" style="display:inline-block;background:#5A1E2D;color:#fff;text-decoration:none;font-size:14px;font-weight:700;padding:13px 24px;border-radius:999px">Ver relatório</a></div><div style="height:1px;background:#EAE1D8;margin-bottom:18px"></div><p style="font-size:13px;margin:0 0 5px;color:#4B423D">Um abraço,</p><p style="font-size:16px;font-weight:700;color:#5A1E2D;margin:0 0 3px">Patrícia Lima</p><p style="font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:#A99B91;margin:0">People Advisory Executive · CALI RH</p></td></tr></table></td></tr></table></body></html>`;
}
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    if(existingOfficial)return json(req,{ok:false,error:"Já existe um fechamento oficial enviado para esta competência. Use uma simulação interna ou o fluxo de correção."},409);
    ...
    const emailRes=await fetch("https://api.resend.com/emails",{method:"POST",headers:{Authorization:`Bearer ${RESEND_API_KEY}`,"Content-Type":"application/json","Idempotency-Key":`workspace-report-${report.id}-v${report.version}`},body:JSON.stringify({from:FROM_EMAIL,to,reply_to:REPLY_TO,subject:`CALI RH · Seu relatório de ${period} está pronto`,html:html(company?.display_name||"sua empresa",period,message,report.id)})});
