...
const CALI_LOGO = '/brand/cali-workspace-transparent.svg';
...
export function ExecutiveReportPaperV17({ company, snapshot, editor, reportType, periodName, protocol, deliveries, approvalIdentity, acknowledgementIdentity, approvedAt, acknowledgedAt, acknowledgementProtocol }: Props) {
  ...
  const hasValidation = Boolean(approval || acknowledgement), hasDirection = Boolean(attentions.length || decisions.length || nextSteps.length || feedback.length), totalPages = 1 + (hasDirection ? 1 : 0) + (hasValidation ? 1 : 0), validationPage = hasDirection ? 3 : 2;
  ...
  return <div className="reports-v16-document"><style>{REPORT_V19_CSS}</style>
    <article className="reports-v16-paper reports-v19-paper reports-v16-paper-one">{header()}
      ...
      {deliveryRows.length ? <section className="reports-v16-execution-grid" style={{ gridTemplateColumns: '1fr' }}><div className="reports-v16-execution-block"><div className="reports-v16-section-head compact"><div><small>EXECUÇÃO DO PERÍODO</small><h2>Entregas e andamento</h2></div></div><table className="reports-v19-table"><thead><tr><th>Entregável</th><th>Frente</th><th>Complex.</th><th>Prazo</th><th>Status</th></tr></thead><tbody>{deliveryRows.map((item) => { const state = deliveryState(item, snapshot.period.end.slice(0, 10)), complexity = (item as DeliveryPerformanceRow & { complexity?: string | null }).complexity; return <tr key={item.deliverable_id}><td><strong>{item.title}</strong>{item.total_minutes ? <small>{formatHoursV14(Number(item.total_minutes))} aplicados</small> : null}</td><td>{item.workstream || '—'}</td><td>{complexity ? <span className="reports-v19-pill">{complexity}</span> : '—'}</td><td>{formatDate(item.effective_due_at)}</td><td><span className={`reports-v19-pill ${state.tone}`}>{state.label}</span>{state.detail ? <small>{state.detail}</small> : null}</td></tr>; })}</tbody></table></div></section> : null}
    ...
    {hasDirection ? <article className="reports-v16-paper reports-v19-paper reports-v16-paper-two">{header('DIREÇÃO DO PERÍODO')}<div className="reports-v16-page-two-title reports-v19-page-title"><span>DA LEITURA À AÇÃO</span><h1>O que precisa de atenção e qual é o próximo movimento.</h1></div>
      ...
      <footer className="reports-v16-footer"><span>CALI Workspace · Patrícia Lima</span><span>{protocol}</span><b>2 / {totalPages}</b></footer>
    </article> : null}
