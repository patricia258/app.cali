...
export function SignatureMarkV55({identity}:{identity?:ReportIdentityV55|null}){if(identity?.signature_mode==='uploaded'&&identity.signature_url)return <img className="report-signature-image-v55" src={identity.signature_url} alt={`Assinatura de ${identity.full_name||'responsável'}`}/>;return <span className="report-signature-generated-v55">{identity?.full_name||'Assinatura'}</span>;}
