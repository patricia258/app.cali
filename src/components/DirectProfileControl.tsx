...
type SignatureStyle='executive'|'editorial'|'notarial'|'heritage'|'calligraphic'|'autograph'|'contemporary'|'italian'|'minimal'|'personal';
type ProfileData={
  full_name:string;email:string;job_title:string;phone:string;whatsapp:string;linkedin_url:string;instagram_url:string;
  avatar_url:string;avatar_position_x:number;avatar_position_y:number;avatar_zoom:number;
...
const signatureStyles:SignatureOption[]=[
  ...
  {value:'editorial',label:'Editorial',hint:'Elegante e de alto contraste'},
  {value:'notarial',label:'Notarial',hint:'Clássica, firme e discreta'},
  {value:'heritage',label:'Heritage',hint:'Caligrafia refinada'},
  {value:'calligraphic',label:'Caligráfica',hint:'Traço longo e leve'},
  {value:'autograph',label:'Autógrafo',hint:'Natural e contínua'},
  {value:'contemporary',label:'Contemporânea',hint:'Limpa e atual'},
  {value:'italian',label:'Italiana',hint:'Fina e inclinada'},
  {value:'minimal',label:'Minimal',hint:'Discreta e elegante'},
  {value:'personal',label:'Personal',hint:'Marcante sem exagero'},
];
...
function whatsappUrl(value:string){const digits=value.replace(/\D/g,'');if(!digits)return'';return`https://wa.me/${digits.startsWith('55')?digits:`55${digits}`}`;}
function avatarStyle(profile:Pick<ProfileData,'avatar_position_x'|'avatar_position_y'|'avatar_zoom'>):CSSProperties{return{'--avatar-x':`${profile.avatar_position_x}%`,'--avatar-y':`${profile.avatar_position_y}%`,'--avatar-zoom':String(profile.avatar_zoom)} as CSSProperties;}
function safeSignatureStyle(value:unknown):SignatureStyle{
  const raw=String(value||'');
  const legacy=legacyStyleMap[raw];
...
export function DirectProfileControl({role}:{role:Role}){
  ...
  async function saveProfile(){
    ...
    try{
      ...
      const user=sessionData.session?.user;
      if(supabase&&user){
        const{error}=await supabase.rpc('update_my_profile_v56',{
  ...
  return <>
    ...
    {modalOpen?<div className="modal-backdrop chrome-modal-backdrop full-screen-modal" role="presentation">
      <section className="modal-card profile-modal profile-modal-v2 profile-modal-v55" role="dialog" aria-modal="true" aria-label="Editar perfil">
        ...
        <div className="profile-photo-workbench"><div className="profile-photo-stage"><span className="profile-avatar profile-avatar-editor large-editor-avatar">{draft.avatar_url?<img src={draft.avatar_url} alt="Prévia do perfil" style={avatarStyle(draft)}/>:<span>{initials(draft.full_name)}</span>}</span><label className="photo-upload-button"><Camera size={17}/>Escolher foto<input type="file" accept="image/png,image/jpeg,image/webp" onChange={handleAvatar}/></label><small>JPG, PNG ou WEBP · até 5 MB · otimização automática</small></div><div className={`profile-crop-controls ${draft.avatar_url?'':'disabled'}`}><label>Zoom<input type="range" min="1" max="3" step="0.05" value={draft.avatar_zoom} disabled={!draft.avatar_url} onChange={(event)=>setDraft((current)=>({...current,avatar_zoom:Number(event.target.value)}))}/></label><label>Horizontal<input type="range" min="0" max="100" value={draft.avatar_position_x} disabled={!draft.avatar_url} onChange={(event)=>setDraft((current)=>({...current,avatar_position_x:Number(event.target.value)}))}/></label><label>Vertical<input type="range" min="0" max="100" value={draft.avatar_position_y} disabled={!draft.avatar_url} onChange={(event)=>setDraft((current)=>({...current,avatar_position_y:Number(event.target.value)}))}/></label></div></div>
