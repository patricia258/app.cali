...
export function AdminClientsPageV3() {
  ...
  async function loadClients() {
    ...
    const rows:ClientRow[] = companies.map((item:any) => {
    ...
    const readyRows=rows.map((row)=>({...row,logoUrl:logoMap.get(row.id)||row.logoUrl||''}));
  ...
  return <Shell role="admin">
    <section className="page clients-page-v3">
      <div className="page-heading"><div className="workspace-page-identity"><span className="workspace-page-icon" aria-hidden="true"><Building2 size={27}/></span><div className="workspace-page-copy"><span className="workspace-page-kicker">CARTEIRA CALI</span><h1>Clientes</h1><p>Gestão da conta, contrato, financeiro, acesso e operação em uma única visão.</p></div></div><button className="primary" onClick={()=>{setForm(emptyForm);setRegistrationTab('data');setCreateOpen(true);}}><Plus size={18}/>Cadastrar cliente</button></div>
