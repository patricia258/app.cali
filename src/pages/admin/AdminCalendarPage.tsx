...
function eventHour(event: WorkspaceCalendarEvent) {
  return new Date(event.startsAt).getHours();
}
...
export function AdminCalendarPage() {
  ...
  useEffect(() => {
    const active = openCreate || Boolean(selectedEvent);
    document.body.classList.toggle('workspace-modal-open', active);
    return () => document.body.classList.remove('workspace-modal-open');
  }, [openCreate, selectedEvent]);

  async function loadCalendar() {
    if (!supabase) return;
    try {
      const [{ data: companyRows }, { data: eventRows }, { data: deadlineRows }, { data: connectionRows }] = await Promise.all([
        ...
        supabase.from('deliverables').select('id, company_id, project_id, title, due_at, status').not('due_at', 'is', null).not('status', 'in', '(approved,cancelled)').order('due_at'),
      ...
      const deadlines: WorkspaceCalendarEvent[] = (deadlineRows || []).map((row: any) => ({
        id: `deadline-${row.id}`,
      ...
      if (manual.length || deadlines.length) setEvents([...manual, ...deadlines].sort((a, b) => new Date(a.startsAt).getTime() - new Date(b.startsAt).getTime()));
  ...
  const visibleEvents = useMemo(() => events.filter((event) => {
    ...
    if (query && !`${event.title} ${event.company || ''} ${calendarTypeMeta[event.type]?.label || ''}`.toLowerCase().includes(query.toLowerCase())) return false;
    return true;
  ...
  async function createEvent(event: FormEvent) {
    ...
    const local: WorkspaceCalendarEvent = {
      id: `local-${Date.now()}`, title: form.title.trim(), companyId: form.companyId || null, company: company?.name || null, companyLogo: company?.logoUrl,
      type: form.type, color: form.color, startsAt, endsAt, allDay: form.allDay, mode: form.mode,
      location: form.location || null, meetingUrl: form.meetingUrl || null, description: form.description || null,
      visibility: form.visibility, attendees: attendeeEmails.map((email) => ({ name: email.split('@')[0], email, status: 'pending' })),
      sourceType: 'manual', syncStatus: calendarConnection === 'connected' ? 'pending' : 'local', synthetic: false,
    };

    try {
      const canPersist = Boolean(supabase && (!form.companyId || /^[0-9a-f-]{36}$/i.test(form.companyId)));
      if (canPersist && supabase) {
        ...
        setEvents((current) => [...current, local].sort((a, b) => new Date(a.startsAt).getTime() - new Date(b.startsAt).getTime()));
      }
      setOpenCreate(false);
    } catch (error) {
  ...
  async function cancelEvent() {
    ...
    if (supabase && !selectedEvent.id.startsWith('local-') && !selectedEvent.id.startsWith('cal-')) {
  ...
  const cursorLabel = view === 'week'
    ? (() => { const start = startOfCalendarWeek(cursor); const end = new Date(start); end.setDate(start.getDate() + 6); return `${start.getDate()} ${months[start.getMonth()].slice(0,3).toLowerCase()} — ${end.getDate()} ${months[end.getMonth()].slice(0,3).toLowerCase()} ${end.getFullYear()}`; })()
    ...
    <Shell role="admin">
      <section className="page calendar-page-v2">
        ...
        <div className="page-heading calendar-page-heading">
          <div><h1>Calendário</h1><p>Reuniões, validações, compromissos e prazos conectados ao trabalho. Prazos de entregáveis entram automaticamente na agenda.</p></div>
        ...
        <section className="calendar-workspace-strip">
          ...
          <div><strong>Google Workspace</strong><p>{calendarConnection === 'connected' ? 'Calendário conectado. Novos eventos podem seguir para sincronização e usar Google Meet.' : 'A estrutura de sincronização está pronta. A conexão OAuth será ativada sem criar uma agenda paralela à sua.'}</p></div>
          <span className={`calendar-connection-status ${calendarConnection === 'connected' ? 'connected' : ''}`}>{calendarConnection === 'connected' ? <><Check size={14} />Conectado</> : 'Não conectado'}</span>
        </section>
        ...
        <section className="calendar-main-toolbar">
          ...
          <div className="calendar-toolbar-filters">
            <label className="calendar-search"><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar evento" /></label>
            <select value={companyFilter} onChange={(event) => setCompanyFilter(event.target.value)} aria-label="Filtrar cliente"><option value="all">Todos os clientes</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select>
            <div className="calendar-view-switch">{(['month','week','agenda'] as CalendarView[]).map((item) => <button key={item} className={view === item ? 'active' : ''} onClick={() => setView(item)}>{item === 'month' ? 'Mês' : item === 'week' ? 'Semana' : 'Agenda'}</button>)}</div>
          </div>
        </section>
        ...
        <div className="calendar-page-layout">
          <section className="calendar-primary-panel panel">
            {view === 'month' && <>
              ...
              <div className="calendar-month-grid">
                {monthDates.map((date) => {
                  ...
                  return <article key={key} className={`calendar-month-cell ${!inMonth ? 'outside' : ''} ${isToday ? 'today' : ''}`} onDoubleClick={() => openCreateForDate(date)}>
                    ...
                    <div className="calendar-cell-events">{dayEvents.slice(0, 3).map((event) => <button key={event.id} className={`calendar-event-chip type-${event.type}`} style={eventStyle(event)} onClick={(click) => { click.stopPropagation(); setSelectedEvent(event); }}><span className="calendar-event-dot" />{!event.allDay && <time>{formatCalendarTime(event.startsAt)}</time>}<strong>{event.title}</strong></button>)}{dayEvents.length > 3 && <button className="calendar-more-events" onClick={() => { setCursor(date); setView('agenda'); }}>+{dayEvents.length - 3} eventos</button>}</div>
            ...
            {view === 'week' && <div className="calendar-week-view">
              ...
              {hours.map((hour) => <div className="calendar-week-row" key={hour} style={{ gridRow: hour - 5 }}><span className="calendar-hour-label">{String(hour).padStart(2,'0')}:00</span>{weekDates.map((date) => {
                const dayEvents = (eventsByDate.get(dateKey(date)) || []).filter((event) => eventHour(event) === hour);
                return <div key={dateKey(date)} className="calendar-week-slot" onDoubleClick={() => { const target = new Date(date); setForm({ ...defaultForm(dateKey(target)), startTime: `${String(hour).padStart(2,'0')}:00`, endTime: `${String(Math.min(hour + 1, 23)).padStart(2,'0')}:00`, companyId: companies[0]?.id || 'aurora' }); setOpenCreate(true); }}>{dayEvents.map((event) => <button key={event.id} className="calendar-week-event" style={eventStyle(event)} onClick={() => setSelectedEvent(event)}><time>{formatCalendarTime(event.startsAt)}</time><strong>{event.title}</strong><small>{event.company || 'CALI'}</small></button>)}</div>;
              })}</div>)}
            </div>}

            {view === 'agenda' && <div className="calendar-agenda-view">{visibleEvents.filter((event) => {
              const date = new Date(event.startsAt); return date.getMonth() === cursor.getMonth() && date.getFullYear() === cursor.getFullYear();
            }).sort((a,b) => new Date(a.startsAt).getTime() - new Date(b.startsAt).getTime()).map((event) => <button key={event.id} className="calendar-agenda-row" onClick={() => setSelectedEvent(event)}><div className="calendar-agenda-date"><strong>{new Date(event.startsAt).getDate()}</strong><span>{months[new Date(event.startsAt).getMonth()].slice(0,3).toUpperCase()}</span></div><span className="calendar-event-bar" style={{ background: event.color }} /><div className="calendar-agenda-copy"><span>{calendarTypeMeta[event.type].label} · {formatCalendarTime(event.startsAt)}</span><strong>{event.title}</strong><small>{event.company || 'CALI'}{event.visibility === 'client' ? ' · cliente convidado' : ' · interno'}</small></div><span className="calendar-agenda-mode">{event.mode === 'in_person' ? <MapPin size={15} /> : <Video size={15} />}{event.mode === 'in_person' ? 'Presencial' : event.mode === 'remote' ? 'Remoto' : 'Prazo'}</span></button>)}</div>}
          </section>
          ...
          <aside className="calendar-side-column">
            <section className="calendar-mini-card panel">
              ...
              <div className="calendar-mini-grid">{miniDates.map((date) => {
            ...
            <section className="calendar-upcoming-card panel">
              ...
              <div className="calendar-upcoming-list">{upcoming.map((event) => <button key={event.id} onClick={() => setSelectedEvent(event)}><span className="calendar-upcoming-color" style={{ background: event.color }} /><div><strong>{event.title}</strong><small>{formatCalendarDate(event.startsAt)} · {formatCalendarTime(event.startsAt)}</small><span>{event.company || 'CALI'}</span></div></button>)}{!upcoming.length && <p>Nenhum compromisso neste recorte.</p>}</div>
            </section>
          </aside>
        </div>
      </section>

      {openCreate && <div className="modal-backdrop full-screen-modal calendar-modal-backdrop">
        <form className="modal-card calendar-event-modal" onSubmit={createEvent} role="dialog" aria-modal="true">
          <button type="button" className="modal-close" onClick={() => setOpenCreate(false)} aria-label="Fechar"><X size={20} /></button>
          <div className="calendar-modal-heading"><span className="section-kicker">NOVO EVENTO</span><h2>Adicionar ao calendário</h2><p>Crie o compromisso uma vez e defina quem deve enxergá-lo. A sincronização com Google Workspace fica vinculada à conexão da sua conta.</p></div>
          <div className="calendar-modal-body">
            ...
            {!form.allDay && <div className="calendar-event-form-grid"><label className="stacked-label">Início<input type="time" value={form.startTime} onChange={(event) => setForm((current) => ({ ...current, startTime: event.target.value }))} /></label><label className="stacked-label">Término<input type="time" value={form.endTime} onChange={(event) => setForm((current) => ({ ...current, endTime: event.target.value }))} /></label><label className="stacked-label">Formato<select value={form.mode} onChange={(event) => setForm((current) => ({ ...current, mode: event.target.value as 'remote'|'in_person' }))}><option value="remote">Remoto</option><option value="in_person">Presencial</option></select></label><label className="stacked-label">Visibilidade<select value={form.visibility} onChange={(event) => setForm((current) => ({ ...current, visibility: event.target.value as 'internal'|'client' }))}><option value="client">Compartilhar com cliente</option><option value="internal">Somente CALI</option></select></label></div>}
            <div className="calendar-event-form-grid"><label className="stacked-label">Local / sala<input value={form.location} onChange={(event) => setForm((current) => ({ ...current, location: event.target.value }))} placeholder={form.mode === 'remote' ? 'Google Meet' : 'Endereço ou sala'} /></label><label className="stacked-label">Link da reunião<input value={form.meetingUrl} onChange={(event) => setForm((current) => ({ ...current, meetingUrl: event.target.value }))} placeholder="https://meet.google.com/..." /></label></div>
          ...
          <div className="calendar-modal-footer"><button type="button" className="secondary" onClick={() => setOpenCreate(false)}>Cancelar</button><button className="primary" disabled={saving || !form.title.trim()} type="submit">{saving ? 'Salvando…' : 'Adicionar evento'}</button></div>
        </form>
      </div>}
      ...
      {selectedEvent && <div className="modal-backdrop full-screen-modal calendar-modal-backdrop">
        <section className="modal-card calendar-detail-modal" role="dialog" aria-modal="true">
          ...
          <div className="calendar-detail-heading"><div className="calendar-detail-company-mark">{getCompanyMark(selectedEvent.company)}</div><div><span className="section-kicker">{calendarTypeMeta[selectedEvent.type].label}</span><h2>{selectedEvent.title}</h2><p>{selectedEvent.company || 'CALI'} · {selectedEvent.visibility === 'client' ? 'visível para o cliente' : 'interno'}</p></div></div>
          <div className="calendar-detail-body">
            <div className="calendar-detail-facts"><article><Clock3 size={17} /><span>Quando</span><strong>{formatCalendarDate(selectedEvent.startsAt)} · {formatCalendarTime(selectedEvent.startsAt)}</strong></article><article>{selectedEvent.mode === 'in_person' ? <MapPin size={17} /> : <Video size={17} />}<span>Formato</span><strong>{selectedEvent.mode === 'in_person' ? 'Presencial' : selectedEvent.mode === 'remote' ? 'Remoto' : 'Prazo automático'}</strong></article><article><Users size={17} /><span>Convidados</span><strong>{selectedEvent.attendees.length ? `${selectedEvent.attendees.length} convidado(s)` : selectedEvent.visibility === 'client' ? 'Cliente relacionado' : 'Somente CALI'}</strong></article></div>
            ...
            {selectedEvent.synthetic && <div className="calendar-auto-source"><CalendarDays size={18} /><div><strong>Prazo automático</strong><p>Este item vem do deadline de um entregável. Para alterar a data, edite o entregável de origem.</p></div></div>}
            ...
            {showCancel && !selectedEvent.synthetic && <section className="calendar-cancel-box"><strong>Cancelar compromisso</strong><p>O motivo fica no histórico e será usado quando ativarmos a régua de notificações aos participantes.</p><textarea value={cancelReason} onChange={(event) => setCancelReason(event.target.value)} rows={2} placeholder="Motivo do cancelamento ou remarcação" /><div><button className="secondary" onClick={() => setShowCancel(false)}>Voltar</button><button className="primary danger-action" disabled={!cancelReason.trim()} onClick={() => void cancelEvent()}>Confirmar cancelamento</button></div></section>}
          </div>
          <div className="calendar-detail-footer"><button className="secondary" onClick={() => window.open(googleCalendarTemplate(selectedEvent), '_blank', 'noopener,noreferrer')}><ExternalLink size={16} />Google Calendar</button><button className="secondary" onClick={() => downloadCalendarIcs(selectedEvent)}><Download size={16} />Baixar .ics</button>{selectedEvent.meetingUrl && <button className="primary" onClick={() => window.open(selectedEvent.meetingUrl!, '_blank', 'noopener,noreferrer')}><Video size={16} />Abrir Meet</button>}{!selectedEvent.synthetic && !showCancel && <button className="secondary danger-soft" onClick={() => setShowCancel(true)}>Cancelar evento</button>}</div>
        </section>
      </div>}
    </Shell>
  );
