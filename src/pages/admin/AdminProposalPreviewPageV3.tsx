...
export function AdminProposalPreviewPageV3(){
  ...
  if(!proposal.pdf_path)return <Shell role="admin"><section className="portal-legacy-proposal-host" aria-label={`Prévia da proposta v${proposal.version}`}><iframe title={`Prévia da proposta v${proposal.version}`} src={`/portal-legacy/proposta.html?id=${encodeURIComponent(proposal.id)}`}/></section></Shell>;
