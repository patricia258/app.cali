...
import '../../admin-projects-stability-v62.css';
...
const statusTone: Record<DeliverableStatus,string> = {
  not_started:'status-neutral', in_progress:'status-progress', standby:'status-standby', internal_review:'status-review',
  client_review:'status-client', adjustment_requested:'status-adjustment', rebriefing:'status-rebriefing',
  approved:'status-approved', cancelled:'status-cancelled',
};
...
const servicePlanLabel = (service?: string, plan?: string | null) => {
  ...
  if (service === 'CALI Full') return 'Assessoria Estratégica Mensal · CALI Full';
...
function previewFronts(project: WorkspaceProject): ProjectFront[] {
  ...
  return Array.from(grouped.entries()).map(([name,items],index) => ({
    ...
    status:items.every((i)=>i.status==='approved')?'completed':items.some((i)=>['in_progress','client_review'].includes(i.status))?'active':'planned', sortOrder:index+1,
  }));
}
...
function previewTasks(deliverable: ProjectDeliverable): TaskItem[] {
  ...
  const finalDate = deliverable.dueAt ? new Date(deliverable.dueAt) : null;
  return names.map((title,index)=>{
    const due = finalDate ? new Date(finalDate) : null;
    if (due) due.setDate(due.getDate() - Math.max(0,(names.length-index-1)*2));
    return { id:`preview-task-${deliverable.id}-${index}`, protocol:`CALI-TSK-PREVIEW-${String(index+1).padStart(3,'0')}`, deliverableId:deliverable.id, title,
      status:index<(deliverable.taskDone||0)?'done':index===(deliverable.taskDone||0)&&deliverable.status==='in_progress'?'in_progress':'todo',
      dueAt:due?.toISOString()||deliverable.dueAt, clientVisible:index===names.length-1, estimatedMinutes:60+(index%3)*30, sortOrder:index+1 };
  });
}
...
function StatusSelect({ item, onChange, compact=false }: { item:ProjectDeliverable; onChange:(status:DeliverableStatus)=>void; compact?:boolean }) {
  return <label className={`inline-status-select-v3 ${statusTone[item.status]} ${compact?'compact':''}`} onClick={(e)=>e.stopPropagation()}>
    <span className="status-dot-v3"/><select aria-label={`Status de ${item.title}`} value={item.status} disabled={item.status==='approved'} onChange={(e)=>onChange(e.target.value as DeliverableStatus)}>
      {statuses.map((status)=><option key={status} value={status}>{deliverableLabels[status]}</option>)}
    </select>
  </label>;
}
...
export function AdminProjectsPageV3() {
  ...
  useEffect(()=>{
    const active=projectModal||frontModal||deliverableModal||Boolean(selectedDeliverable)||Boolean(pendingStatus)||confirmDelete;
    document.body.classList.toggle('workspace-modal-open',active);
    return()=>document.body.classList.remove('workspace-modal-open');
  },[projectModal,frontModal,deliverableModal,selectedDeliverable,pendingStatus,confirmDelete]);
  useEffect(()=>{
  ...
  const summary=useMemo(()=>({active:projects.filter((p)=>p.planningStatus==='active').length,waiting:projects.reduce((s,p)=>s+p.deliverables.filter((d)=>d.status==='client_review').length,0),rebriefing:projects.reduce((s,p)=>s+p.deliverables.filter((d)=>d.rebriefingRequired||d.status==='rebriefing').length,0),impacted:projects.reduce((s,p)=>s+p.deliverables.filter((d)=>Boolean(d.originalDueAt)||d.clientDelayBusinessDays>0).length,0)}),[projects]);
  ...
  function statusChoice(item:ProjectDeliverable,next:DeliverableStatus){
    if(next===item.status||item.status==='approved')return;
    if(next==='adjustment_requested'){
      setSelectedDeliverable(item); setDetailTab('overview'); setShowAdjustment(true); return;
    }
    if(['standby','rebriefing','cancelled'].includes(next)){
      setPendingStatus({item,status:next}); setStatusReason(''); return;
    }
    void persistStatus(item,next,'');
  }

  async function persistStatus(item:ProjectDeliverable,next:DeliverableStatus,note:string){
    if(item.status==='approved'||next===item.status)return;
    if(supabase&&isUuid(item.id)){
      const {error}=await supabase.rpc('change_deliverable_status',{p_deliverable_id:item.id,p_status:next,p_note:note||null});
      if(error){console.error(error);return;}
      await loadWorkspace();
      if(selectedDeliverable?.id===item.id) await loadDeliverableContext(item.id);
    } else {
      setProjects((c)=>c.map((p)=>({...p,planningStatus:next==='rebriefing'&&p.id===selectedProject.id?'rebriefing':p.planningStatus,deliverables:p.deliverables.map((d)=>d.id===item.id?{...d,status:next,rebriefingRequired:next==='rebriefing'?true:d.rebriefingRequired}:d)})));
      setHistory((c)=>[{id:`preview-history-${Date.now()}`,title:`${deliverableLabels[item.status]} → ${deliverableLabels[next]}`,detail:note||'Mudança registrada no fluxo.',createdAt:new Date().toISOString()},...c]);
    }
    setSelectedDeliverable((current)=>current?.id===item.id?{...current,status:next,rebriefingRequired:next==='rebriefing'?true:current.rebriefingRequired}:current);
  }

  async function confirmPendingStatus(){ if(!pendingStatus||!statusReason.trim())return; const current=pendingStatus; setPendingStatus(null); await persistStatus(current.item,current.status,statusReason.trim()); setStatusReason(''); }
  ...
  async function requestAdjustment(){
    ...
    if(supabase&&isUuid(selectedDeliverable.id)){ const {error}=await supabase.rpc('request_deliverable_adjustment',{p_deliverable_id:selectedDeliverable.id,p_reason:adjustmentReason.trim(),p_impact_business_days:adjustmentImpact}); if(error)console.error(error); else { await loadWorkspace(); await loadDeliverableContext(selectedDeliverable.id); } }
    else setProjects((c)=>c.map((p)=>p.id===selectedProject.id?{...p,planningStatus:rebrief?'rebriefing':p.planningStatus,deliverables:p.deliverables.map((d)=>d.id===selectedDeliverable.id?{...d,status:rebrief?'rebriefing':'adjustment_requested',adjustmentCount:next,rebriefingRequired:rebrief}:d)}:p));
    setSelectedDeliverable((c)=>c?{...c,status:rebrief?'rebriefing':'adjustment_requested',adjustmentCount:next,rebriefingRequired:rebrief}:c); setAdjustmentReason('');setAdjustmentImpact(0);setShowAdjustment(false);
  }
  ...
  async function startTimer(){
    ...
    if(selectedDeliverable.status==='not_started')void persistStatus(selectedDeliverable,'in_progress','Timer iniciado');
  }
  ...
  async function deleteDeliverable(){
    if(!selectedDeliverable||selectedDeliverable.status!=='not_started')return;
    if(supabase&&isUuid(selectedDeliverable.id)){await supabase.from('deliverables').delete().eq('id',selectedDeliverable.id);await loadWorkspace();}
    else setProjects((c)=>c.map((p)=>({...p,deliverables:p.deliverables.filter((d)=>d.id!==selectedDeliverable.id)})));
    setConfirmDelete(false);setSelectedDeliverable(null);
  }
  ...
  return <Shell role="admin"><section className="page projects-flow-page">
    <div className="page-heading projects-heading-v2"><div className="workspace-page-identity"><span className="workspace-page-icon" aria-hidden="true"><FolderKanban size={27}/></span><div className="workspace-page-copy"><span className="workspace-page-kicker">EXECUÇÃO & ROADMAP</span><h1>Projetos</h1><p>Organize o cronograma por frentes, desdobre cada frente em entregáveis e acompanhe etapas, conversas, horas e validações sem perder a rastreabilidade.</p></div></div><button className="primary" onClick={openNewProject}><Plus size={18}/>Novo cronograma</button></div>

    <section className="project-kpis-v2"><article><span className="project-kpi-icon"><FolderKanban size={18}/></span><div><span>Projetos vigentes</span><strong>{summary.active}</strong><small>em operação</small></div></article><article className="project-kpi-attention"><span className="project-kpi-icon"><Clock3 size={18}/></span><div><span>Aguardando cliente</span><strong>{summary.waiting}</strong><small>pedem acompanhamento</small></div></article><article><span className="project-kpi-icon"><RefreshCw size={18}/></span><div><span>Rebriefing</span><strong>{summary.rebriefing}</strong><small>escopos em revisão</small></div></article><article><span className="project-kpi-icon"><AlertTriangle size={18}/></span><div><span>Prazos impactados</span><strong>{summary.impacted}</strong><small>com dependência aberta</small></div></article></section>
    ...
    {projectView==='roadmap'&&<section className="roadmap-v2 panel"><header><div><span className="section-kicker">SEQUÊNCIA DE IMPLANTAÇÃO</span><h2>Frentes do cronograma</h2><p>Meses, dependências e entregáveis organizados em uma única sequência.</p></div></header><div className="roadmap-months-v2"><span>Frente / entregáveis</span>{months.map((m)=><b key={m}>M{m}</b>)}</div><div className="front-list-v2">{selectedFronts.map((front,index)=>{const ds=selectedProject.deliverables.filter((d)=>d.workstream===front.name||(d as any).workstreamId===front.id).filter((d)=>d.status!=='cancelled');return <section className="front-section-v2" key={front.id}><div className="front-header-v2"><img src={index%2?'/brand/cali-lime-mark.svg':'/brand/cali-oak-mark.svg'} alt=""/><div className="front-copy-v2"><span>{front.protocol}</span><strong>{front.name}</strong><p>{front.objective||'Frente do roadmap CALI.'}</p></div><div className="front-monthbar-v2" style={{'--months':months.length} as any}>{months.map((m)=><i key={m}/>)}<b style={{gridColumn:`${front.monthStart||1} / ${Math.min(months.length+1,(front.monthEnd||front.monthStart||1)+1)}`}}>M{front.monthStart||1}{front.monthEnd&&front.monthEnd!==front.monthStart?`–M${front.monthEnd}`:''}</b></div><button className="secondary" onClick={()=>openDeliverable(front)}><Plus size={15}/>Entregável</button></div><div className="front-deliverables-v2 front-deliverables-v3">{ds.length?ds.map((item)=><div className="front-deliverable-row-v3" key={item.id}><button className="front-deliverable-open-v3" onClick={()=>{setSelectedDeliverable(item);setDetailTab('overview');}}><FileText size={18}/><span><small>{item.protocol}</small><strong>{item.title}</strong></span><b className={`mc-chip ${item.complexity.toLowerCase()}`} title={complexityMeta[item.complexity].description} aria-label={`${item.complexity}: ${complexityMeta[item.complexity].description}`}>{item.complexity}</b></button><StatusSelect item={item} compact onChange={(next)=>statusChoice(item,next)}/><time>{formatProjectDate(item.dueAt)}</time><button className="row-arrow-v3" onClick={()=>{setSelectedDeliverable(item);setDetailTab('overview');}} aria-label={`Abrir ${item.title}`}><ArrowRight size={17}/></button></div>):<div className="front-empty-v2">Nenhum entregável nesta frente ainda.</div>}</div></section>})}</div></section>}

    {projectView==='deliverables'&&<><div className="deliverable-toolbar-v2"><div><button className={deliverableView==='list'?'active':''} onClick={()=>setDeliverableView('list')}><List size={16}/>Lista</button><button className={deliverableView==='kanban'?'active':''} onClick={()=>setDeliverableView('kanban')}><LayoutGrid size={16}/>Kanban</button></div><span>{selectedProject.deliverables.filter((d)=>d.status!=='cancelled').length} entregáveis</span></div>{deliverableView==='list'?<section className="deliverable-list-v2 deliverable-list-v3 panel">{selectedProject.deliverables.filter((d)=>d.status!=='cancelled').map((item)=><div className="deliverable-list-row-v3" key={item.id}><button className="deliverable-list-open-v3" onClick={()=>{setSelectedDeliverable(item);setDetailTab('overview');}}><span className="deliverable-icon-v2">{item.isDocument?<FileText size={19}/>:<CheckCircle2 size={19}/>}</span><span className="deliverable-copy-v2"><small>{item.protocol}</small><strong>{item.title}</strong><em>{item.workstream}</em></span><b className={`mc-chip ${item.complexity.toLowerCase()}`} title={complexityMeta[item.complexity].description} aria-label={`${item.complexity}: ${complexityMeta[item.complexity].description}`}>{item.complexity}</b></button><StatusSelect item={item} onChange={(next)=>statusChoice(item,next)}/><span className="task-count-v2">{item.taskDone}/{item.taskCount} etapas</span><span>{hoursLabel(item.hours)}</span><time>{formatProjectDate(item.dueAt)}</time><button className="row-arrow-v3" onClick={()=>{setSelectedDeliverable(item);setDetailTab('overview');}}><ArrowRight size={17}/></button></div>)}</section>:<section className="kanban-v2">{statuses.filter((s)=>!['cancelled','standby'].includes(s)).map((status)=><div key={status}><header><strong>{deliverableLabels[status]}</strong><span>{selectedProject.deliverables.filter((d)=>d.status===status).length}</span></header>{selectedProject.deliverables.filter((d)=>d.status===status).map((item)=><button key={item.id} onClick={()=>{setSelectedDeliverable(item);setDetailTab('overview');}}><small>{item.protocol}</small><strong>{item.title}</strong><p>{item.workstream}</p><footer><b className={`mc-chip ${item.complexity.toLowerCase()}`} title={complexityMeta[item.complexity].description} aria-label={`${item.complexity}: ${complexityMeta[item.complexity].description}`}>{item.complexity}</b><time>{formatProjectDate(item.dueAt)}</time></footer></button>)}</div>)}</section>}</>}
  ...
  {selectedDeliverable&&<div className="modal-backdrop full-screen-modal"><section className="modal-card deliverable-workspace-modal-v2"><header className="deliverable-workspace-header-v2"><div className="deliverable-big-icon">{selectedDeliverable.isDocument?<FileText size={25}/>:<FolderKanban size={25}/>}</div><div className="deliverable-title-v2"><span className="section-kicker">{selectedDeliverable.protocol}</span><h2>{selectedDeliverable.title}</h2><p>{selectedDeliverable.workstream} · {selectedDeliverable.complexity} · {roadmapLabel(selectedDeliverable)}</p></div><div className="deliverable-head-actions-v2"><span className={`status-chip-v3 ${statusTone[selectedDeliverable.status]}`}>{deliverableLabels[selectedDeliverable.status]}</span><button className="modal-close-static" onClick={()=>{setSelectedDeliverable(null);setShowAdjustment(false);}}><X size={23}/></button></div></header><nav className="deliverable-tabs-v2">{(['overview','tasks','conversation','history'] as DetailTab[]).map((tab)=><button key={tab} className={detailTab===tab?'active':''} onClick={()=>setDetailTab(tab)}>{tab==='overview'?'Visão geral':tab==='tasks'?`Subtarefas (${selectedTasks.length})`:tab==='conversation'?`Conversa (${comments.length})`:'Histórico'}</button>)}</nav><div className="deliverable-workspace-scroll-v2">{detailTab==='overview'&&<><div className="deliverable-summary-lines-v2"><div><CalendarDays size={19}/><span>Deadline</span><strong>{formatProjectDate(selectedDeliverable.dueAt)}</strong>{selectedDeliverable.originalDueAt&&<small>Original: {formatProjectDate(selectedDeliverable.originalDueAt)}</small>}</div><div><Clock3 size={19}/><span>Horas</span><strong>{hoursLabel(selectedDeliverable.hours)}</strong><small>registradas</small></div><div><RefreshCw size={19}/><span>Ajustes</span><strong>{selectedDeliverable.adjustmentCount}/{selectedProject.adjustmentLimit}</strong><small>{selectedDeliverable.rebriefingRequired?'Rebriefing necessário':'com justificativa'}</small></div><div><GitBranch size={19}/><span>Frente</span><strong>{selectedDeliverable.workstream}</strong><small>{roadmapLabel(selectedDeliverable)}</small></div></div>{selectedDeliverable.clientDelayBusinessDays>0&&<div className="deliverable-warning-v2"><AlertTriangle size={18}/><div><strong>Resposta do cliente em atraso</strong><p>{selectedDeliverable.clientDelayBusinessDays} dia(s) útil(eis) de impacto. O atraso altera as próximas deadlines e fica registrado no histórico.</p></div></div>}<section className="deliverable-description-v2"><div><strong>Complexidade</strong><span className={`mc-chip ${selectedDeliverable.complexity.toLowerCase()}`}>{selectedDeliverable.complexity}</span></div><p>{complexityMeta[selectedDeliverable.complexity].description}</p>{selectedDeliverable.description&&<p>{selectedDeliverable.description}</p>}</section>{showAdjustment&&<form className="adjustment-form-v2" onSubmit={(e)=>{e.preventDefault();void requestAdjustment();}}><strong>Registrar pedido de alteração</strong><p>Até o 3º pedido é ajuste. O 4º passa automaticamente para rebriefing.</p><textarea rows={3} value={adjustmentReason} onChange={(e)=>setAdjustmentReason(e.target.value)} placeholder="Justificativa obrigatória"/><label>Impacto na deadline<input type="number" min={0} max={90} value={adjustmentImpact} onChange={(e)=>setAdjustmentImpact(Number(e.target.value))}/><span>dias úteis</span></label><div><button type="button" className="secondary" onClick={()=>setShowAdjustment(false)}>Cancelar</button><button className="primary" disabled={!adjustmentReason.trim()}>Registrar ajuste</button></div></form>}</>}
    ...
    {detailTab==='conversation'&&<section className="conversation-pane-v2 workspace-chat-standard"><header><div className="conversation-channels-v2"><button className={conversationChannel==='client'?'active':''} onClick={()=>setConversationChannel('client')}>Cliente</button><button className={conversationChannel==='internal'?'active':''} onClick={()=>setConversationChannel('internal')}>Interno CALI</button></div><span>{conversationChannel==='client'?'Mensagens compartilhadas com o cliente':'Notas internas, invisíveis ao cliente'}</span></header><div className="conversation-list-v2 workspace-chat-list">{comments.filter((m)=>m.clientVisible===(conversationChannel==='client')).map((m)=><article className="workspace-chat-fallback-line" key={m.id}><span className="conversation-avatar-v2">PL</span><div className="workspace-chat-fallback-bubble"><header><strong>{m.author||'Patrícia · CALI'}</strong><time>{new Date(m.createdAt).toLocaleString('pt-BR')}</time></header><p>{m.body}</p></div></article>)}{!comments.some((m)=>m.clientVisible===(conversationChannel==='client'))&&<div className="workspace-chat-empty">Nenhuma mensagem neste canal.</div>}</div><div className="conversation-composer-v2 workspace-chat-compose"><textarea rows={3} value={messageText} onChange={(e)=>setMessageText(e.target.value)} placeholder={conversationChannel==='client'?'Escreva uma mensagem para o cliente…':'Registre uma nota interna…'}/><input ref={fileInputRef} type="file" hidden onChange={(e)=>setMessageFile(e.target.files?.[0]||null)}/><div><button className="secondary" onClick={()=>fileInputRef.current?.click()}><Paperclip size={16}/>{messageFile?messageFile.name:'Anexar'}</button><button className="primary" onClick={()=>void sendMessage()} disabled={!messageText.trim()&&!messageFile}><Send size={16}/>Enviar</button></div></div></section>}
  ...
  </div>{detailTab!=='conversation'&&<footer className="deliverable-actions-v2"><div className="deliverable-actions-left-v2">{selectedDeliverable.status!=='approved'&&selectedDeliverable.status!=='cancelled'&&<button className="secondary" onClick={()=>openDeliverable(undefined,selectedDeliverable)}><Edit3 size={16}/>Editar</button>}<StatusSelect item={selectedDeliverable} onChange={(next)=>statusChoice(selectedDeliverable,next)}/>{selectedDeliverable.status!=='approved'&&selectedDeliverable.status!=='cancelled'&&<button className="secondary" onClick={()=>setShowAdjustment(true)}><RefreshCw size={16}/>Ajuste</button>}<button className="secondary danger-text" onClick={()=>{setPendingStatus({item:selectedDeliverable,status:'cancelled'});setStatusReason('');}} disabled={selectedDeliverable.status==='approved'||selectedDeliverable.status==='cancelled'}><X size={16}/>Cancelar</button><button className="secondary danger-text" onClick={()=>setConfirmDelete(true)} disabled={selectedDeliverable.status!=='not_started'}><Trash2 size={16}/>Excluir</button></div><div className="deliverable-actions-right-v2">{selectedDeliverable.status==='in_progress'&&<button className="secondary" onClick={()=>void persistStatus(selectedDeliverable,'client_review','Enviado para validação do cliente')}><Send size={16}/>Enviar ao cliente</button>}{selectedDeliverable.status==='approved'?<span className="locked-v2"><ShieldCheck size={16}/>Aprovado e protegido</span>:activeTimer?.deliverableId===selectedDeliverable.id?<button className="timer-button-v2 active" onClick={()=>void stopTimer()}><Square size={15}/>Parar timer · {timerLabel}</button>:<button className="timer-button-v2" onClick={()=>void startTimer()} disabled={Boolean(activeTimer)}><Play size={16}/>Iniciar timer</button>}</div></footer>}</section></div>}

  {pendingStatus&&<div className="modal-backdrop full-screen-modal"><section className="modal-card confirm-project-modal-v2"><button className="modal-close" onClick={()=>setPendingStatus(null)}><X size={20}/></button><header><span className="section-kicker">ALTERAR STATUS</span><h2>{deliverableLabels[pendingStatus.status]}</h2><p>Registre o motivo. Ele ficará associado à data/hora da mudança no histórico e poderá alimentar o relatório do projeto.</p></header><textarea rows={4} value={statusReason} onChange={(e)=>setStatusReason(e.target.value)} placeholder={pendingStatus.status==='standby'?'Por que este entregável ficará em standby?':pendingStatus.status==='rebriefing'?'O que exige rebriefing?':'Motivo do cancelamento'}/><footer><button className="secondary" onClick={()=>setPendingStatus(null)}>Voltar</button><button className="primary" disabled={!statusReason.trim()} onClick={()=>void confirmPendingStatus()}>Confirmar status</button></footer></section></div>}

  {confirmDelete&&selectedDeliverable&&<div className="modal-backdrop full-screen-modal"><section className="modal-card confirm-project-modal-v2"><button className="modal-close" onClick={()=>setConfirmDelete(false)}><X size={20}/></button><header><span className="section-kicker">CONFIRMAÇÃO</span><h2>Excluir entregável</h2><p>A exclusão só é permitida antes do início. Depois disso, o correto é cancelar para preservar o histórico.</p></header><footer><button className="secondary" onClick={()=>setConfirmDelete(false)}>Voltar</button><button className="primary danger-button" onClick={()=>void deleteDeliverable()}>Excluir</button></footer></section></div>}
  </Shell>;
}
