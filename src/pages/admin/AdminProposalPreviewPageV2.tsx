...
const present=(v:any)=>String(v??'').replace(/sponsor\s*\/\s*diretoria/gi,'liderança responsável ou diretoria').replace(/\bsponsor\b/gi,'liderança responsável');
const list=(v:any):string[]=>Array.isArray(v)?v.map(present).filter(Boolean):[];
...
function scopeGroups(items:any[]){const groups:Array<{number:number;text:string;subitems:string[]}>=[];for(const raw of items){const item=present(raw),isSub=/^\s*[-–—]\s*/.test(item),text=item.replace(/^\s*[-–—]\s*/,'');if(isSub&&groups.length)groups.at(-1)!.subitems.push(text);else groups.push({number:groups.length+1,text,subitems:[]})}return groups}
...
function paragraphs(value:string,limit=3){const raw=present(value).replace(/\s+/g,' ').trim();if(!raw)return[];const manual=present(value).split(/\n+/).map(x=>x.trim()).filter(Boolean);if(manual.length>1)return manual.slice(0,limit);const sentences=raw.match(/[^.!?]+[.!?]+|[^.!?]+$/g)?.map(x=>x.trim()).filter(Boolean)||[raw];if(sentences.length<=1)return[raw];const buckets:string[]=[];let current='';for(const sentence of sentences){if((current+' '+sentence).trim().length>245&&current){buckets.push(current.trim());current=sentence}else current=(current+' '+sentence).trim()}if(current)buckets.push(current.trim());return buckets.slice(0,limit)}
function unique(items:string[]){return [...new Set(items.map(x=>present(x).trim()).filter(Boolean))]}
function descriptor(slug:string){const map:Record<string,string>={'assessoria-estrategica':'People Advisory recorrente','cali-build':'Estruturação assistida de RH','mentoria-rh':'Desenvolvimento para profissionais de RH','diagnostico-executivo':'Diagnóstico executivo de People','cultura-direcao':'Cultura e direção organizacional','shadowing-lideranca':'Desenvolvimento aplicado de liderança',treinamentos:'Treinamentos e experiências de aprendizagem','marca-empregadora':'Marca empregadora e experiência','solucao-personalizada':'Solução desenhada sob medida'};return map[slug]||portalServiceLabel(slug)}
...
export function AdminProposalPreviewPageV2(){
 ...
 const calc=proposal.calculator_data||{},packageInfo=(PACKAGE_META[proposal.service_slug]||[]).find(x=>x.code===proposal.package_code),hours=Number(calc.monthlyHours||packageInfo?.suggestedHours||0),months=Number(proposal.contract_months||packageInfo?.minimumMonths||1),profile=getProposalProfile(proposal.service_slug,proposal.package_code,proposal.submission.answers||{},months,hours),scope=scopeGroups(proposal.scope_items),cycles=Array.isArray(calc.cycles)?calc.cycles.map((x:any)=>({...x,title:present(x.title),focus:present(x.focus),objective:present(x.objective),duration:present(x.duration)})):[],roadmap=list(calc.roadmapItems),out=list(calc.outOfScope),pain=list(calc.painPoints),results=list(calc.expectedResults),advantages=list(calc.advantages),cadence=list(calc.cadence),cali=list(calc.caliResponsibilities),client=list(calc.clientResponsibilities),map=calc.mapaPeople,payment=paymentRows(proposal),bonus=calc.bonus||null,nextSteps=(Array.isArray(calc.nextSteps)&&calc.nextSteps.length?calc.nextSteps:profile.nextSteps).map((x:any)=>[present(x[0]),present(x[1])] as [string,string]),monthly=Boolean(calc.monthly||proposal.service_slug==='assessoria-estrategica'||proposal.service_slug==='cali-build'||(proposal.service_slug==='marca-empregadora'&&proposal.package_code==='RECORRENTE')),firstName=proposal.submission.contact_name.trim().split(/\s+/)[0]||'Olá',reference=Number(proposal.subtotal||proposal.final_unit||0),discount=Math.max(0,reference-Number(proposal.final_unit||0)),commercial=unique(list(calc.commercialConditions).length?list(calc.commercialConditions):profile.commercial||[]),operating=unique([...(profile.operating||[]).map(present),...cadence]),companyName=proposal.submission.company_name||'Sua empresa',solutionCopy=proposal.public_notes||profile.solutionCopy,solutionTagline=present(calc.solutionTagline)||descriptor(proposal.service_slug),contextParagraphs=paragraphs(calc.contextSummary||'Contexto em revisão.',3),readingParagraphs=paragraphs(calc.executiveReading||'',2),solutionParagraphs=paragraphs(solutionCopy||'',2),whats=`https://wa.me/5541987791933?text=${encodeURIComponent(`Olá, Pati! Aqui é ${proposal.submission.contact_name||''}, da ${proposal.submission.company_name||''}. Analisei a proposta ${packageInfo?.label||proposal.package_code} (${proposal.submission.protocol}) e gostaria de conversar sobre ela.`)}`,total=5;
 const operatingClean=operating.filter(x=>!/carga mensal contratada|\b\d+\s*horas\b|contrato mínimo|mensalidade remunera|atuação 100% online|passagens|hospedagem|alimentação|deslocamentos/i.test(x)).slice(0,4);
 const roadmapOrCycles=roadmap.length?roadmap:cycles.map((x:any)=>[x.title,x.focus].filter(Boolean).join(' — ')).filter(Boolean);

 return <Shell role="admin"><section className="page portal-proposal-preview"><header className="portal-proposal-preview-toolbar"><button className="secondary" onClick={()=>navigate(`/admin/propostas/${proposal.submission_id}/editar`)}><ArrowLeft size={16}/>Editar proposta</button><div><strong>Prévia da proposta · modelo editorial</strong><span>Confira as cinco páginas A4 antes de salvar e enviar.</span></div><div><button className="secondary" onClick={print}><Printer size={16}/>Salvar PDF</button><button className="primary" onClick={()=>setSendOpen(true)}><Mail size={16}/>Enviar por e-mail</button></div></header>{error&&<div className="inline-notice danger">{error}</div>}{proposal.status==='enviada'&&proposal.sent_at&&<div className="inline-notice">Versão enviada em {date(proposal.sent_at)}. Uma nova edição criará outra versão sem alterar esta.</div>}<div className="portal-proposal-document">

 <Page proposal={proposal} index={1} total={total} className="proposal-cover" darkHeader>
   <div className="proposal-cover-band">
     <div className="proposal-kicker light">Proposta CALI RH</div>
     <h1>Olá, {firstName}.</h1>
     <p className="cover-recipient">Preparamos esta proposta para <strong>{companyName}</strong>, a partir do contexto compartilhado no briefing.</p>
     <div className="cover-service">
       <span>{descriptor(proposal.service_slug)}</span>
       <strong>{packageInfo?.label||portalServiceLabel(proposal.service_slug)}</strong>
     </div>
   </div>
   <section className="proposal-context-block">
     <div className="section-heading compact"><span>01</span><div><small>Leitura do contexto</small><h2>O que entendemos até aqui</h2></div></div>
     <div className="editorial-copy">{contextParagraphs.map((x,i)=><p key={i}>{x}</p>)}</div>
   </section>
   {pain.length>0&&<section className="context-signals"><span>Sinais que orientam esta proposta</span><div>{pain.slice(0,4).map((x,i)=><b key={i}>{x}</b>)}</div></section>}
   <div className="cover-bottom-grid single">
     {map?.include&&Number(map.score)>0&&<aside className="map-result editorial"><span>Mapa de People</span><strong>{Number(map.score).toLocaleString('pt-BR',{minimumFractionDigits:1,maximumFractionDigits:1})}<small>/10</small></strong><b>{map.quadrant}</b></aside>}
     <aside className="proposal-note"><span>Leitura CALI</span>{readingParagraphs.length?readingParagraphs.map((x,i)=><p key={i}>{x}</p>):<p>A proposta foi organizada a partir do briefing, da capacidade interna e do resultado esperado para este ciclo.</p>}</aside>
   </div>
 </Page>

 <Page proposal={proposal} index={2} total={total} className="proposal-solution">
   <div className="solution-service-card">
     <span className="solution-service-mark" aria-hidden="true"/>
     <div className="solution-service-number">02</div>
     <div className="solution-service-copy"><small>Solução recomendada</small><h1>{packageInfo?.label||proposal.package_code}</h1>{solutionTagline&&<p>{solutionTagline}</p>}</div>
   </div>
   <div className="solution-statement">{solutionParagraphs.map((x,i)=><p key={i}>{x}</p>)}</div>
   <div className="solution-grid editorial">
     <section><span>Por que agora</span><p>{present(calc.whyNow)||'O momento pede clareza de prioridade, método e uma sequência de execução que preserve a capacidade interna.'}</p></section>
     <section><span>Objetivo deste ciclo</span><p>{present(calc.cycleObjective)||'Transformar a prioridade central em um movimento claro, aplicável e sustentável.'}</p></section>
   </div>
 ...
 <Page proposal={proposal} index={3} total={total} className="proposal-delivery">
   <div className="section-heading"><span>03</span><div><small>Escopo contratado</small><h1>O que entra neste trabalho</h1><p>O escopo abaixo traduz a prioridade desta versão em entregas claras, sem misturar o que pertence a etapas futuras.</p></div></div>
   <section className="scope-section"><ol className="scope-list editorial">{scope.map(x=><li key={x.number}><span>{String(x.number).padStart(2,'0')}</span><div><strong>{x.text}</strong>{x.subitems.length>0&&<ul>{x.subitems.map((s,i)=><li key={i}>{s}</li>)}</ul>}</div></li>)}</ol></section>
   {operatingClean.length>0&&<section className="operating-section"><div className="subheading"><small>Ritmo e formato</small><h2>Como a operação acontece</h2></div><div className="operating-cards">{operatingClean.map((x,i)=><div key={i}><span>{String(i+1).padStart(2,'0')}</span><p>{x}</p></div>)}</div></section>}
   {cycles.length>0&&<section className="cycle-section"><div className="subheading"><small>Sequência prevista</small><h2>Ciclos e fases</h2></div><div className="cycle-list editorial">{cycles.slice(0,6).map((c:any,i:number)=><div key={i}><span>{String(i+1).padStart(2,'0')}</span><div><strong>{c.title}</strong>{c.duration&&<small>{c.duration}</small>}{c.focus&&<p>{c.focus}</p>}</div></div>)}</div></section>}
 </Page>

 <Page proposal={proposal} index={4} total={total} className="proposal-governance">
   <div className="section-heading"><span>04</span><div><small>Governança do trabalho</small><h1>Responsabilidades, continuidade e limites</h1><p>Uma boa proposta também deixa claro quem conduz cada parte, o que depende da empresa e o que não está incluído nesta contratação.</p></div></div>
   ...
   {roadmapOrCycles.length>0&&<section className="roadmap-editorial"><div className="subheading"><small>Continuidade</small><h2>O que fica organizado para depois</h2></div><div className="roadmap-list">{roadmapOrCycles.slice(0,5).map((x,i)=><div key={i}><span>{String(i+1).padStart(2,'0')}</span><p>{present(x)}</p></div>)}</div></section>}
   {out.length>0&&<section className="limits-editorial"><div className="subheading"><small>Limites desta proposta</small><h2>O que não está incluído</h2></div><ul>{out.slice(0,5).map((x,i)=><li key={i}>{x}</li>)}</ul></section>}
 </Page>

 <Page proposal={proposal} index={5} total={total} className="proposal-commercial">
   <div className="section-heading"><span>05</span><div><small>Condições comerciais</small><h1>Investimento e próximos passos</h1><p>O desenho comercial abaixo corresponde ao escopo, à capacidade reservada e ao prazo desta versão.</p></div></div>
   <section className="investment-card">
     <div className="investment-main"><span>{monthly?'Mensalidade':'Investimento'}</span><strong>{money(proposal.final_unit)}</strong>{monthly&&<small>por mês</small>}</div>
     <div className="investment-meta">
       <div><span>Referência</span><strong>{money(reference)}</strong></div>
       {discount>0&&<div><span>{present(calc.discountType)||'Condição aplicada'}</span><strong>− {money(discount)}</strong>{calc.discountDescription&&<small>{present(calc.discountDescription)}</small>}</div>}
     </div>
   </section>
