...
export function AdminPeopleMapPageV2() {
  ...
  async function loadAll() {
    ...
    if (!journeysResult.error) setJourneys((journeysResult.data || []) as JourneyRecord[]);
    setLoading(false);
  }
  ...
  const filteredJourneys = useMemo(() => journeys.filter((item) => journeyFilter === 'todos' || journeyStatus(item) === journeyFilter || (journeyFilter === 'copiou' && Number(item.copy_events || 0) > 0)), [journeys, journeyFilter]);
  ...
  const journeyKpis = useMemo(() => ({
    visitas: journeys.length,
    iniciados: journeys.filter((item) => item.started).length,
    abandonos: journeys.filter((item) => journeyStatus(item) === 'abandono').length,
    copias: journeys.reduce((sum, item) => sum + Number(item.copy_events || 0), 0),
  }), [journeys]);

  function openRecord(record: MapaRecord) {
    setSelected(record); setDraft(observationDraft(record)); setFeedback(''); document.body.classList.add('workspace-modal-open');
  }
  function closeRecord() { document.body.classList.remove('workspace-modal-open'); setSelected(null); setDraft(null); setFeedback(''); }

  async function persist(record: MapaRecord, nextStatus: string, nextObservations: Record<string, string>) {
    if (!supabase) return false;
    const { data, error: rpcError } = await supabase.rpc('update_mapa_people_record', {
      p_id: record.id,
      p_status: normalizeStatus(nextStatus),
      p_observacoes: nextObservations,
    });
  ...
  async function updateStatus(record: MapaRecord, nextStatus: string) {
    const ok = await persist(record, nextStatus, record.observacoes || {});
    if (!ok) return;
  ...
  return <Shell role="admin">
    <section className="page people-map-page">
      ...
      <div className="page-heading people-map-heading">
        <div><h1>Mapa de People</h1><p>O Mapa público continua no endereço atual. Aqui ficam a leitura real das respostas, a jornada e a revisão administrativa.</p></div>
      ...
      {tab === 'respostas' ? <>
        ...
        <div className="people-map-toolbar panel"><div className="people-map-search"><Workflow size={18}/><span>Jornada do formulário público</span></div><label className="people-map-filter"><Filter size={17}/><select value={journeyFilter} onChange={(event) => setJourneyFilter(event.target.value)}><option value="todos">Todas as jornadas</option><option value="concluido">Enviou</option><option value="andamento">Em andamento</option><option value="abandono">Abandonou</option><option value="copiou">Copiou conteúdo</option></select></label></div>
    ...
    {selected && draft && selectedScores && <div className="workspace-modal-layer people-map-modal-layer" role="presentation"><section className="workspace-standard-modal people-map-modal" role="dialog" aria-modal="true"><header className="people-map-modal-header"><button className="people-map-back" type="button" onClick={closeRecord}><ChevronLeft size={18}/>Voltar ao Mapa</button><div className="people-map-modal-identity"><div><span className="eyebrow">{selected.protocolo}</span><h2>{selected.c_empresa}</h2><p>{selected.c_nome}{selected.c_cargo ? ` · ${selected.c_cargo}` : ''} · respondido em {formatDate(selected.created_at)}</p></div><div className="people-map-score-hero"><strong>{fmtScore(selectedScores.total)}</strong><span>/10</span><small>score geral</small></div></div></header><div className="people-map-modal-scroll"><div className="people-map-detail-grid"><main className="people-map-dimensions">{([[1,'D1 · Maturidade Estrutural',selectedScores.d1,'d1_obs'],[2,'D2 · Liderança & Cultura',selectedScores.d2,'d2_obs'],[3,'D3 · Dados & Decisão',selectedScores.d3,'d3_obs'],[4,'D4 · Dimensões Operacionais',selectedScores.d4,'d4_obs']] as const).map(([dimension,title,score,key]) => <section className="people-map-dimension-card" key={dimension}><div className="people-map-dimension-head"><div><span className="section-kicker">DIMENSÃO {dimension}</span><h3>{title}</h3></div><strong>{fmtScore(score)}<small>/10</small></strong></div><div className="people-map-answer-grid">{dimensionRows(selected,dimension).map(([label,value]) => <div key={label}><span>{label}</span><strong>{scoreLabel(value)}</strong></div>)}</div><label className="stacked-label">Observação da Patrícia<textarea rows={3} value={draft[key]} onChange={(event) => setDraft((current) => current ? {...current,[key]:event.target.value} : current)}/></label></section>)}<section className="people-map-dimension-card people-map-final-reading"><div className="people-map-dimension-head"><div><span className="section-kicker">DEVOLUTIVA</span><h3>Parecer final</h3></div><FileText size={22}/></div><label className="stacked-label">O que você indicaria primeiro<textarea rows={5} value={draft.parecer} onChange={(event) => setDraft((current) => current ? {...current,parecer:event.target.value} : current)}/></label><label className="stacked-label">Serviço recomendado<select value={draft.servico_recomendado} onChange={(event) => setDraft((current) => current ? {...current,servico_recomendado:event.target.value} : current)}>{SERVICES.map((service) => <option key={service}>{service}</option>)}</select></label></section></main><aside className="people-map-contact-column"><section className="people-map-side-card"><div className="people-map-side-title"><UserRound size={19}/><h3>Contato</h3></div><dl><div><dt>Nome</dt><dd>{selected.c_nome}</dd></div><div><dt>Empresa</dt><dd>{selected.c_empresa}</dd></div><div><dt>Cargo</dt><dd>{selected.c_cargo || 'Não informado'}</dd></div><div><dt>E-mail</dt><dd>{selected.c_email}</dd></div><div><dt>WhatsApp</dt><dd>{selected.c_whatsapp}</dd></div><div><dt>Preferência</dt><dd>{selected.c_preferencia_contato || 'Não informada'}</dd></div></dl></section><section className="people-map-side-card"><div className="people-map-side-title"><Building2 size={19}/><h3>Qualificação</h3></div><dl><div><dt>Prazo</dt><dd>{selected.q_prazo || '—'}</dd></div><div><dt>Decisores</dt><dd>{[...(selected.q_decisor || []),selected.q_decisor_outro].filter(Boolean).join(', ') || '—'}</dd></div><div><dt>Formato</dt><dd>{selected.q_formato || '—'}</dd></div><div><dt>Investimento</dt><dd>{selected.q_investimento || '—'}</dd></div><div><dt>Origem</dt><dd>{selected.q_origem || '—'}</dd></div></dl></section><section className="people-map-side-card people-map-side-actions"><h3>Próximas ações</h3><button className="secondary" type="button" disabled title="Entra na próxima etapa da paridade"><FileText size={16}/>Gerar relatório</button><button className="secondary" type="button" disabled title="Entra junto com o PDF aprovado"><Mail size={16}/>Enviar por e-mail</button><button className="secondary" type="button" onClick={() => { const digits=(selected.c_whatsapp||'').replace(/\D/g,''); const number=(digits.length===10||digits.length===11)?`55${digits}`:digits; if(number) window.open(`https://wa.me/${number}`,'_blank'); }}><Send size={16}/>Abrir WhatsApp</button></section></aside></div></div><footer className="people-map-modal-footer"><span>{feedback}</span><div><button className="secondary" type="button" onClick={closeRecord}>Cancelar</button><button className="primary" type="button" onClick={saveReview} disabled={saving}>{saving ? 'Salvando…' : 'Salvar revisão'}</button></div></footer></section></div>}
  </Shell>;
}
