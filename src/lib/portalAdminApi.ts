...
export async function loadPortalOpportunity(id:string){
  const [submissions,proposals,pricing]=await Promise.all([
    request<PortalSubmission[]>(`cali_submissions?id=eq.${encodeURIComponent(id)}&select=*`),
    request<PortalProposal[]>(`cali_proposals?submission_id=eq.${encodeURIComponent(id)}&select=*&order=version.desc`),
    request<PortalPricingRule[]>('cali_pricing_rules?select=*&active=eq.true&order=service_slug,sort_order'),
  ]);
  const submission=submissions[0]||null;
  let mapa:PortalMapaResponse|null=null;
  if(submission?.contact_email){
    const email=encodeURIComponent(submission.contact_email.trim().toLowerCase());
    try{const rows=await request<PortalMapaResponse[]>(`mapa_respostas?c_email=ilike.${email}&select=id,protocolo,c_email,created_at,diagnostico_v2&order=created_at.desc&limit=1`);mapa=rows[0]||null;}catch{mapa=null;}
  }
...
export async function savePortalProposal(input:{proposalId?:string|null;payload:PortalProposalWrite}){
  ...
  if(input.proposalId){
    const rows=await request<PortalProposal[]>(`cali_proposals?id=eq.${encodeURIComponent(input.proposalId)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify(body)});
    return rows[0]||null;
  }
  const rows=await request<PortalProposal[]>('cali_proposals',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(body)});
  return rows[0]||null;
}
...
export async function updatePortalProposal(id:string,patch:Partial<Pick<PortalProposal,'status'|'pdf_path'|'resend_email_id'|'sent_at'>>){
  const rows=await request<PortalProposal[]>(`cali_proposals?id=eq.${encodeURIComponent(id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({...patch,updated_at:new Date().toISOString()})});
  return rows[0]||null;
}
...
export async function sendPortalProposal(input:{proposal_id:string;pdf_base64:string;pdf_name:string;note:string;company_name:string;recipient_name:string;recipient_email:string}){
  const token=await sessionToken();
  const response=await fetch(`${SUPABASE_URL}/functions/v1/portal-send-proposal`,{method:'POST',headers:{apikey:SUPABASE_KEY,Authorization:`Bearer ${token}`,'Content-Type':'application/json'},body:JSON.stringify(input)});
  const result=await response.json().catch(()=>({}));
  if(!response.ok)throw new Error(result.error||'Não foi possível enviar a proposta.');
  return result as {ok:true;email_id?:string;to?:string;subject?:string};
}
...
export async function appendPortalActivity(input:{submission_id:string;proposal_id?:string|null;event_type:string;metadata?:Record<string,unknown>}){
  const rows=await request<PortalActivity[]>('cali_activity',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({...input,metadata:input.metadata||{}})});
  return rows[0]||null;
}

export function portalServiceLabel(slug:string){
  const labels:Record<string,string>={
    'assessoria-estrategica':'Assessoria Estratégica Mensal — HR as a Service','mentoria-rh':'Mentoria para Profissionais de RH','diagnostico-executivo':'Diagnóstico Executivo de Pessoas','cultura-direcao':'Projeto de Cultura e Direção','shadowing-lideranca':'Shadowing de Liderança',treinamentos:'Treinamentos & Palestras','marca-empregadora':'Marca Empregadora','solucao-personalizada':'Solução Personalizada',
  };return labels[slug]||slug;
}

export const PORTAL_STATUS:Array<{value:PortalSubmissionStatus;label:string}>=[
  {value:'novo',label:'Nova resposta'},{value:'analise',label:'Em análise'},{value:'edicao',label:'Proposta em edição'},{value:'aprovada',label:'Aprovada internamente'},{value:'enviada',label:'Enviada'},{value:'negociacao',label:'Em negociação'},{value:'fechada',label:'Fechada'},{value:'recusada',label:'Recusada'},{value:'expirada',label:'Expirada'},
];
