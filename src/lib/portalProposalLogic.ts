...
export function trainingPackageRecommendation(answers:Answers={}) {
  ...
  if(requested==='treinamento')return{code:'TREINAMENTO',reasons:['Treinamento de desenvolvimento indicado no briefing']};
  if(requested==='workshop')return{code:'WORKSHOP',reasons:['Workshop aplicado indicado no briefing']};
  if(requested==='campanha'||requested==='palestra_tema')return{code:'PALESTRA',reasons:[requested==='campanha'?'Palestra vinculada a campanha ou data do calendário':'Palestra temática indicada no briefing']};
  const meetings=Number(answers.encontros||1),hours=Number(answers.carga_horaria||1.5),interaction=String(answers.nivel_interacao||''),audience=Array.isArray(answers.publico)?answers.publico.map(String):[];
  if(meetings>=4&&(audience.includes('liderancas')||audience.includes('diretoria')))return{code:'PROGRAMA',reasons:[`${meetings} encontros`,'Público de liderança']};
  if(meetings>1)return{code:'TREINAMENTO',reasons:[`${meetings} encontros`,'Desenvolvimento em mais de uma etapa']};
  ...
  return{code:'PALESTRA',reasons:['Encontro único','Duração compatível com palestra']};
}
...
export function scopeDefaults(serviceSlug:string,packageCode:string){
  const map:Record<string,string[]>= {
    ...
    treinamentos:packageCode==='PALESTRA'?['Reunião breve de briefing com o sponsor','Palestra estratégica de 60 a 90 minutos','Conteúdo contextualizado ao público e ao negócio','Facilitação ao vivo por Patrícia Lima','Material-síntese de apoio']:packageCode==='WORKSHOP'?['Reunião de briefing com o sponsor','Workshop aplicado de 2 a 4 horas','Conteúdo, exercícios ou dinâmica conectados ao contexto real','Facilitação ao vivo por Patrícia Lima','Material de apoio e compromissos de aplicação']:packageCode==='PROGRAMA'?['Reunião de briefing e definição da trilha de liderança','Programa estruturado a partir de 4 encontros','Conteúdo aplicado a desafios reais de liderança','Prática e acompanhamento de evolução entre etapas','Síntese final com compromissos e próximos movimentos']:['Reunião de briefing e desenho da competência prioritária','Mais de um encontro personalizado','Conteúdo, exercícios e prática entre os encontros','Facilitação ao vivo por Patrícia Lima','Síntese de aplicação e próximos compromissos'],
