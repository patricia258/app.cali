...
export const INVESTMENT_BANDS:Record<string,{period:string;options:InvestmentBand[]}>= {
  ...
  treinamentos:{period:'pela contratação',options:[{value:'ate25',label:'Até R$ 2,5 mil',min:0,max:2500},{value:'25a4',label:'R$ 2,5 mil a R$ 4 mil',min:2500,max:4000},{value:'4a6',label:'R$ 4 mil a R$ 6 mil',min:4000,max:6000},{value:'6a10',label:'R$ 6 mil a R$ 10 mil',min:6000,max:10000},{value:'10mais',label:'Acima de R$ 10 mil',min:10000,max:null},{value:'avaliar',label:'Prefiro avaliar pelo escopo',min:null,max:null}]},
...
export function investmentContextFor(serviceSlug:string,answers:Answers={}){
  if(serviceSlug==='cali-build'){
    const value=String(answers.budget_status||'');if(!value)return null;
    const max=value==='conheco'&&Number(answers.budget_teto)>0?Number(answers.budget_teto):null;
    const labels:Record<string,string>={conheco:'Orçamento informado pelo lead',existe_sem_valor:'Existe orçamento, valor não compartilhado',propostas_primeiro:'Diretoria quer avaliar propostas primeiro',nao_definido:'Orçamento ainda não definido'};
    return{value,label:labels[value]||value,min:null,max,period:'por mês',open:max===null};
  }
...
export function trainingPackageRecommendation(answers:Answers={}) {
  const requested=String(answers.tipo_contratacao||'');
  if(requested==='palestra'||requested==='campanha'||requested==='palestra_tema')return{code:'PALESTRA',reasons:[answers.campanha_vinculada==='sim'||requested==='campanha'?'Palestra vinculada a campanha ou data do calendário':'Palestra estratégica indicada no briefing']};
  if(requested==='workshop')return{code:'WORKSHOP',reasons:['Workshop aplicado indicado no briefing']};
  if(requested==='treinamento')return{code:'TREINAMENTO',reasons:['Treinamento personalizado indicado no briefing']};
  if(requested==='programa_lideranca')return{code:'PROGRAMA',reasons:['Programa / trilha de liderança indicado no briefing']};
  const recurrence=String(answers.recorrencia_esperada||''),meetings=Number(answers.encontros||(recurrence==='4mais'?4:recurrence==='2a3'?2:1)),hours=Number(answers.carga_horaria||1.5),interaction=String(answers.nivel_interacao||''),audience=Array.isArray(answers.publico)?answers.publico.map(String):[];
  if(recurrence==='4mais'||(meetings>=4&&(audience.includes('liderancas')||audience.includes('diretoria'))))return{code:'PROGRAMA',reasons:[recurrence==='4mais'?'Necessidade de quatro ou mais encontros':`${meetings} encontros`,'Público de liderança']};
  if(recurrence==='2a3'||meetings>1)return{code:'TREINAMENTO',reasons:[recurrence==='2a3'?'Necessidade de dois ou três encontros':`${meetings} encontros`,'Desenvolvimento em mais de uma etapa']};
  if(interaction==='dinamica')return{code:'WORKSHOP',reasons:['Dinâmica ou exercício prático previsto']};
  if(hours>=2)return{code:'WORKSHOP',reasons:[`Duração prevista de ${hours}h`,'Espaço para aplicação prática']};
  return{code:'PALESTRA',reasons:['Ação única','Formato compatível com palestra']};
}
...
export function initialPackageFor(serviceSlug:string,answers:Answers={},pricing:PortalPricingRule[]=[]){
  ...
  if(serviceSlug==='cali-build'){
    const size=Number(answers.colaboradores||0),people=Number(answers.pessoas_rh||0),fronts=Array.isArray(answers.frentes)?answers.frentes.length:0,ratio=people>0?size/people:size;
...
export function calculateProposal(input:{serviceSlug:string;answers:Answers;packageCode:string;basePrice:number|string;discount?:number|string;extras?:number|string;months?:number|string;finalOverride?:number|string|null;scopeMode?:'integral'|'prioritized'}){
  ...
  if(serviceSlug==='assessoria-estrategica'){const size=n(answers.colaboradores,20),sizeFactor=size<=20?1:size<=50?1.08:size<=100?1.15:1.22;const fronts=answers.frentes?.length||1,included=packageCode==='FULL'?2:1,frontFactor=1+Math.max(0,fronts-included)*.06,cadenceFactor=answers.frequencia==='semanal'?1.14:answers.frequencia==='quinzenal'?1.06:1;factor=sizeFactor*frontFactor*cadenceFactor;breakdown.push(['Porte',sizeFactor],['Frentes',frontFactor],['Cadência',cadenceFactor]);const minimumMonths=packageCode==='FULL'?12:8;months=Math.max(months||minimumMonths,minimumMonths);}
  else if(serviceSlug==='cali-build'){
    const size=n(answers.colaboradores,20),people=n(answers.pessoas_rh,0),fronts=Array.isArray(answers.frentes)?answers.frentes.length:0,included=packageCode==='COMPLETO'?3:1;
    const sizeDelta=size<=50?0:size<=100?.05:size<=250?.12:size<=500?.20:size<=1000?.28:.35;
    const scopeDelta=answers.escopo_build==='rh_completo'?.10:answers.escopo_build==='multiplas_frentes'?.07:0;
    const frontDelta=Math.min(.15,Math.max(0,fronts-included)*.03);
    const capacityDelta=answers.capacidade_execucao==='menos2'?.08:answers.capacidade_execucao==='2a4'?.05:answers.capacidade_execucao==='5a8'?.02:0;
    const ratio=people>0?size/people:size,ratioDelta=ratio>250?.10:ratio>150?.07:ratio>100?.04:0;
    const cadenceDelta=answers.cadencia_build==='semanal'?.05:answers.cadencia_build==='quinzenal'?.02:0,startingDelta=answers.ponto_partida==='zero'?.04:0;
    ...
    const minimumMonths=packageCode==='COMPLETO'?6:4;months=Math.max(months||minimumMonths,minimumMonths);
  }
...
export function scopeDefaults(serviceSlug:string,packageCode:string){
  const map:Record<string,string[]>= {
    ...
    'cali-build':packageCode==='COMPLETO'?['Arquitetura-alvo e roadmap de implantação por ciclos','Métodos, critérios, templates e referências para as frentes priorizadas','Revisões técnicas frequentes das entregas produzidas pelo RH interno','Checkpoints executivos para aprovações e remoção de impedimentos','Acompanhamento da adoção e transferência de conhecimento para sustentação interna']:['Diagnóstico do ponto de partida e arquitetura da frente prioritária','Método, critérios, templates e referências para o primeiro ciclo','Checkpoints de orientação e revisão técnica das entregas internas','Apoio à validação com sponsor ou diretoria nos marcos necessários','Roadmap de sustentação e próximas frentes após o ciclo'],
    ...
    treinamentos:packageCode==='PALESTRA'?['Reunião breve de briefing com o sponsor','Palestra estratégica de 60 ou 90 minutos','Conteúdo contextualizado ao público e ao negócio','Facilitação ao vivo por Patrícia Lima','Material-síntese de apoio']:packageCode==='WORKSHOP'?['Reunião de briefing com o sponsor','Workshop aplicado de 2 a 4 horas','Conteúdo, exercícios ou dinâmica conectados ao contexto real','Facilitação ao vivo por Patrícia Lima','Material de apoio e compromissos de aplicação']:packageCode==='PROGRAMA'?['Reunião de briefing e definição da trilha de liderança','Programa estruturado de 4 a 10 encontros','Conteúdo aplicado a desafios reais de liderança','Prática e acompanhamento de evolução entre etapas','Síntese final com compromissos e próximos movimentos']:['Reunião de briefing e desenho da competência prioritária','Treinamento personalizado em 2 ou 3 encontros','Conteúdo, exercícios e prática entre os encontros','Facilitação ao vivo por Patrícia Lima','Síntese de aplicação e próximos compromissos'],
